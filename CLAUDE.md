...
**The core invariant**: LLM only fires after a human commits attention.
...
Everything else (concept extraction, edges, misconceptions, tasks) is downstream of those three modes.
...
```
sources                    PDF/text inputs + topic_anchors_json + llm_filter_keep_ids_json
...
concept_edges              requires | enables | related | causes | contrasts_with | example_of
...
- Heavy passes (enrich, concepts, misconceptions, tasks, grader, lazy_tasks): use `heavyModel`
...
3. Decisions are **term-keyed** (stable across re-extracts), not id-keyed
4. Persisted as normalized terms in `sources.llm_filter_keep_ids_json` (legacy numeric saves auto-wiped on read)
