...
describe('candidate shared helpers', () => {
  it('classifies quality flags before confidence bands', () => {
    ...
    expect(classifyBucket(candidate({ confidence: 0.95, topic_relevance_score: 0 }))).toBe('high');
    expect(classifyBucket(candidate({ confidence: 0.7, topic_relevance_score: 0.1 }))).toBe('off_topic');
  });

  it('keeps bulk promote conservative', () => {
    expect(passesBulkPromoteGate(Object.assign(candidate(), { bucket: 'high' as const }))).toBe(true);
    expect(passesBulkPromoteGate(Object.assign(candidate({ mention_count: 1 }), { bucket: 'high' as const }))).toBe(false);
