...
export default function App() {
  ...
  const [reviewCounts, setReviewCounts] = useState<{ newCount: number; dueCount: number }>({ newCount: 0, dueCount: 0 });
  ...
  useEffect(() => {
    const refreshDue = () => {
      window.api.review.dueCount().then(c => setReviewCounts(c as { newCount: number; dueCount: number }));
    };
    refreshDue();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'transparent', color: '#e2e8f0', fontFamily: 'system-ui, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <header style={{ height: 48, padding: '0 20px', borderBottom: '1px solid rgba(35,42,85,0.65)', display: 'flex', alignItems: 'center', background: 'rgba(4,6,26,0.72)', backdropFilter: 'blur(18px)', flexShrink: 0, gap: 12, position: 'relative', zIndex: 1 }}>
        ...
        {(reviewCounts.newCount + reviewCounts.dueCount) > 0 && (() => {
          // Honest label: brand-new and due-now are distinct review states, so
          // we never call a new card "due". Mirrors the queue's `N new · M due`.
          const label = [
            reviewCounts.newCount > 0 ? `${reviewCounts.newCount} new` : null,
            reviewCounts.dueCount > 0 ? `${reviewCounts.dueCount} due` : null,
          ].filter(Boolean).join(' · ');
          return (
            <button
              onClick={() => { setTopLevel('review'); setSelectedConcept(null); }}
              title={`Review queue: ${label}`}
              aria-label={`Review queue: ${label}`}
              style={{
